...
router.post('/in', authMiddleware, async (req, res, next) => {
  try {
    ...
    if (reward.reward_type === 'money') {
      await db.query('UPDATE `user` SET `money` = `money` + ? WHERE `id` = ?', [reward.reward_value, uid]);
      ...
      let newExp = user.exp + reward.reward_value;
      ...
      rewards.push(`经验+${reward.reward_value}`);
      if (leveled) rewards.push('🎉升级！');
    } else if (reward.reward_type === 'gold') {
      await db.query('UPDATE `user` SET `gold` = `gold` + ? WHERE `id` = ?', [reward.reward_value, uid]);
      rewards.push(`金币+${reward.reward_value}`);
    } else if (reward.reward_type === 'item') {
      const existing = await db.getOne(
        'SELECT * FROM `inventory` WHERE `user_id` = ? AND `item_id` = ? AND `equipped` = 0',
        [uid, reward.reward_value]
      );
      if (existing) {
        await db.query('UPDATE `inventory` SET `quantity` = `quantity` + ? WHERE `id` = ?', [reward.reward_qty, existing.id]);
      } else {
        await db.insert('inventory', { user_id: uid, item_id: reward.reward_value, quantity: reward.reward_qty, equipped: 0, enhance_level: 0 });
      }
      const item = await db.getOne('SELECT `name` FROM `item` WHERE `id` = ?', [reward.reward_value]);
      rewards.push(`${item ? item.name : '物品'} x${reward.reward_qty}`);
    }
