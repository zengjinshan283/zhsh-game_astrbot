...
- battle.js 已引用 statusUtil.processTickEffects()
- **未完成**：驱散道具实际调用 / 状态叠加规则 / 状态图标UI / 战斗外debuff
...
- **问题**：城内建筑点击无反应，无法进入建筑交互面板

### 4.8 NPC系统 🟡 基础完成
- NPC对话 `/npc/:id`
- **待完善**：NPC交互面板（当前点击无响应），功能选项（铁匠强化/商店购买等）

### 4.9 商城系统 🔴 定价Bug
- mall.js 用 gold 定价，需改 money
- 玩家 gold=0，无法购买任何商品
...
### 4.12 福利中心 ✅ 基础完成
- 欢迎礼包 / 每日首次分享 / 等级礼包
- claim-starter 有debug代码残留（fs=require + console.error）
...
| # | 功能 | 现状 | 目标 | 参考沧澜四海 |
|---|------|------|------|------------|
| 1 | **商城定价修复** | mall.js用gold | 改money | — |
| 2 | **城内建筑交互** | 点击无反应 | 弹出交互面板 | 铁匠铺/杂货铺/银行/酒馆等可点击 |
| 3 | **套装属性** | 无 | 2件/3件/5件套触发额外属性 | 破碎の斗魂5件套 |
| 4 | **debuff完整实现** | 部分完成 | 回合扣血/状态图标/驱散 | 中毒/灼烧/冰冻 |

### 🟡 中优先级（提升深度）

| # | 功能 | 现状 | 目标 |
|---|------|------|------|
| 5 | **特性天赋系统** | 无 | 每角色可点天赋树（沧澜四海参考） |
| 6 | **宠物自动成长** | 静态宠物 | 宠物参战后自动升级/加属性 |
| 7 | **副本次数限制** | 无限刷 | 每日/每周限制+银币重置 |
| 8 | **航海立即完成** | 等待计时 | 消耗道具立即完成 |
| 9 | **新手装备设计** | 通用 | 参考沧澜四海8件1级套装 |
| 10 | **支线任务内容** | 占位符多 | 填充具体目标/怪物/奖励 |
